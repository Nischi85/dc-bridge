...
| `POST /repair` `{release_dir, missing_files}` | Human-triggered only (called by media-audit's UI, never automatically): search the hub for a source with the EXACT same release name as `release_dir`'s own folder, then re-queue that WHOLE folder to its existing location — not a per-file download. AirDC++ itself skips what it already has and fetches the rest; a per-file download was tried first and found to fail outright (AirDC++'s own share index can mark a search result "already have this" even when specific files are genuinely missing, refusing the download). Used to patch a release media-audit's `sfv_integrity` check found missing volumes in. See `dcbridge/repair.py` for the identity-matching rules and the full story. |
